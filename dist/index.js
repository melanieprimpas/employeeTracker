...
function printCustomTable(data) {
    ...
    const columnWidths = headers.map(header => Math.max(...data.map(row => String(row[header] || '').length), header.length));
    ...
    selectMethod() {
        inquirer
            ...
            if (answers.options === 'View all departments') {
                ...
                pool.query(`SELECT r.id, r.title, d.name, r.salary
                FROM roles r
                JOIN departments d ON r.department_id = d.id`, (err, result) => {
                    if (err) {
                        console.log(err);
                    }
                    else if (result) {
                ...
                pool.query(`SELECT e.id, e.first_name, e.last_name, r.title, d.name AS department, r.salary, m.first_name || ' '|| m.last_name as manager 
                FROM employees e
              ...
              VALUES ($1)`, [answer.newDepartment]), (err, result) => {
                        if (err) {
                            console.log(err);
                        }
                        else if (result) {
